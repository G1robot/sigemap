<div class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-60 backdrop-blur-sm z-50 animate-fade-in-down p-4">
    <div class="max-w-xl w-full bg-white rounded-2xl shadow-2xl overflow-hidden border-t-4 border-green-500">
        ...
        <form wire:submit.prevent="guardarAsistencia" class="p-6">
            <p class="text-sm font-bold text-gray-600 uppercase mb-4 border-b pb-2">Marcar personal presente:</p>
            
            <div class="space-y-3 mb-6 max-h-[300px] overflow-y-auto pr-2">
                @foreach($datos_asignacion->detallesCuadrilla as $detalle)
                    <label class="flex items-center p-3 border rounded-lg cursor-pointer transition-all hover:bg-green-50 {{ $asistencias[$detalle->id_usuario] ? 'border-green-400 bg-green-50' : 'border-gray-200 bg-white' }}">
                        <input type="checkbox" wire:model="asistencias.{{ $detalle->id_usuario }}" class="w-5 h-5 text-green-600 border-gray-300 rounded focus:ring-green-500">
            ...
            <div class="flex justify-end gap-3 pt-2">
                <button type="button" wire:click="closeAsistenciaModal" class="px-5 py-2.5 text-sm font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg">Cerrar</button>
