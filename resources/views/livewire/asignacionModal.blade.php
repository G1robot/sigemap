<div class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 backdrop-blur-sm z-40 animate-fade-in-down p-4">
    <div class="max-w-5xl w-full bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-full">
        
        <div class="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center">
            <h2 class="text-lg font-bold text-gray-800"><i class="fa-solid fa-calendar-plus mr-2 text-orange-500"></i> Nueva Planificación</h2>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="date" wire:model="fecha" class="w-full border border-gray-300 rounded-lg p-2.5 focus:ring-2 focus:ring-orange-500 text-sm">
                        @error('fecha') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Ruta *</label>
                        <select wire:model.live="id_ruta" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm bg-white">
                            <option value="">Seleccionar ruta...</option>
                            @foreach($rutas as $ruta)
                                <option value="{{ $ruta->id_ruta }}">{{ $ruta->nombre_ruta }}</option>
                            @endforeach
                        </select>
                        @error('id_ruta') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Turno (Automático)</label>
                        <input type="text" wire:model="turno" readonly placeholder="Se llena al elegir ruta..." class="w-full border border-gray-200 bg-gray-100 text-gray-500 rounded-lg p-2.5 text-sm cursor-not-allowed font-bold">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-gray-600 uppercase mb-1">Vehículo *</label>
                        <select wire:model="id_camion" class="w-full border border-gray-300 rounded-lg p-2.5 text-sm bg-white">
                            <option value="">Seleccionar camión...</option>
                            @foreach($camiones as $camion)
                                <option value="{{ $camion->id_camion }}">{{ $camion->placa }} - {{ $camion->dimension_tipo }}</option>
                            @endforeach
                        </select>
                        @error('id_camion') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div class="flex flex-col h-full border-l border-gray-100 pl-0 lg:pl-6">
                    ...
                    <div class="flex gap-2 mb-4">
                        <div class="flex-1">
                            <select wire:model="trabajador_seleccionado" class="w-full border border-gray-300 rounded-lg p-2 text-sm bg-white">
                    ...
                    <div class="flex-1 bg-gray-50 rounded-lg border border-gray-200 p-2 overflow-y-auto min-h-[150px]">
                        @forelse($cuadrilla as $index => $miembro)
                            <div class="flex justify-between items-center bg-white p-2 mb-2 rounded border border-gray-100 shadow-sm">
                                <div>
                                    <p class="text-sm font-bold text-gray-800">{{ $miembro['nombre'] }}</p>
                                    <p class="text-xs text-gray-500">{{ $miembro['rol_en_viaje'] }}</p>
                                </div>
                                <button wire:click="quitarTrabajador({{ $index }})" class="text-gray-400 hover:text-red-500 px-2"><i class="fa-solid fa-xmark"></i></button>
                            </div>
                        @empty
        ...
        <div class="bg-gray-50 px-6 py-4 flex justify-end gap-3 border-t border-gray-100">
            ...
            <button wire:click="guardarPlanificacion" wire:loading.attr="disabled" wire:target="guardarPlanificacion" class="px-5 py-2.5 text-sm font-bold text-white bg-orange-500 hover:bg-orange-600 rounded-lg flex items-center gap-2">
