<div class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-60 backdrop-blur-sm z-50 animate-fade-in-down p-4">
    <div class="max-w-md w-full bg-white rounded-2xl shadow-2xl overflow-hidden border-t-4 border-gray-800">
        
        <div class="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center">
            <h2 class="text-lg font-bold text-gray-800"><i class="fa-solid fa-weight-scale mr-2 text-gray-700"></i> Registro de Pesaje</h2>
        ...
        <form wire:submit.prevent="guardarDescarga" class="p-6">
            ...
            <div class="bg-orange-50 rounded-lg p-3 mb-5 border border-orange-100 flex items-center justify-between">
                <div>
                    <p class="text-xs text-orange-600 font-bold uppercase">Camión</p>
                    <p class="text-lg font-black text-gray-800">{{ $camion_placa }}</p>
                </div>
                <div class="text-right">
                    <p class="text-xs text-orange-600 font-bold uppercase">Proviene de</p>
            ...
            <div class="mb-6">
                ...
                <div class="relative">
                    <input type="number" step="0.1" wire:model="peso_descargado" 
                        class="w-full pl-4 pr-12 py-3 border-2 border-gray-300 rounded-xl focus:ring-0 focus:border-gray-800 text-xl font-bold text-center transition-colors">
                    <div class="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none">
                        <span class="text-gray-500 font-bold">Ton</span>
                    </div>
                </div>
                @error('peso_descargado') <span class="text-red-500 text-xs font-bold mt-1 block text-center">{{ $message }}</span> @enderror
            ...
            <div class="mb-8 space-y-3">
                ...
                <div wire:click="$set('accion_post_descarga', 'retornar')" 
                     class="flex items-center p-3 border-2 rounded-xl cursor-pointer transition-all {{ $accion_post_descarga === 'retornar' ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:bg-gray-50' }}">
                    <div class="w-5 h-5 rounded-full border-2 flex items-center justify-center {{ $accion_post_descarga === 'retornar' ? 'border-blue-500' : 'border-gray-300' }}">
                        @if($accion_post_descarga === 'retornar') <div class="w-2.5 h-2.5 bg-blue-500 rounded-full"></div> @endif
                    </div>
                    <div class="ml-3">
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit" wire:loading.attr="disabled" wire:target="guardarDescarga" class="px-5 py-3 text-sm font-bold text-white bg-gray-800 hover:bg-black rounded-lg shadow-md flex items-center gap-2 transition-all">
