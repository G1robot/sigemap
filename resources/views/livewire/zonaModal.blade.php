<div class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-60 backdrop-blur-sm z-50 animate-fade-in-down p-4">
    <div class="max-w-lg w-full mx-4 bg-white rounded-2xl shadow-2xl overflow-hidden border-t-4 border-emap-blue">
        
        <div class="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center">
            <h2 class="text-lg font-bold text-gray-800">
                <i class="fa-solid fa-map-location-dot mr-2 text-emap-blue"></i>
                {{ $zona_id ? 'Editar Zona Territorial' : 'Registrar Nueva Zona' }}
            </h2>
            ...
            <div class="space-y-5 mb-6">
                <div>
                    ...
                    <input wire:model="nombre_zona" type="text" autocomplete="off" placeholder="Ej: Zona Alta, Casco Viejo, Plan 40..."
                        class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-emap-blue transition-shadow text-base">
                    @error('nombre_zona') <span class="text-red-500 text-xs mt-1 block font-bold">{{ $message }}</span> @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea wire:model="descripcion" rows="3" placeholder="Ej: Abarca desde la Av. Murillo hasta la plaza principal..."
                        class="w-full border border-gray-300 rounded-lg p-2.5 focus:outline-none focus:ring-2 focus:ring-emap-blue transition-shadow resize-none"></textarea>
                    @error('descripcion') <span class="text-red-500 text-xs mt-1 block font-bold">{{ $message }}</span> @enderror
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 border-t border-gray-100 pt-5">
                ...
                <button type="submit" 
                    ...
                    <span wire:loading.remove wire:target="enviarClick" class="flex items-center gap-2">
                        <i class="fa-solid fa-save"></i> {{ $zona_id ? 'Guardar Cambios' : 'Registrar Zona' }}
                    </span>
